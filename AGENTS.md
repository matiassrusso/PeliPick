...
**Butaca** — motor de recomendaciones de pelis/series basado en el gusto real de una persona (no en promedios genéricos). Stack: FastAPI + SQLite local / Postgres en producción (backend), React + Vite + Tailwind (frontend, tema "cinematic"). Repo: [github.com/matiassrusso/Butaca](https://github.com/matiassrusso/Butaca). Deploy: frontend [pelipick.vercel.app](https://pelipick.vercel.app/) (Vercel), backend [pelipick-backend.onrender.com](https://pelipick-backend.onrender.com) (Render, free tier — cold start en la primera request). Local para desarrollo: backend puerto 8001, frontend 4173.
...
**Estado actual** (detalle completo en `docs/mvp-status.md` y `docs/build-log.md`): activo, motor de recomendación personalizado por perfil real de usuario (Fase 1 del plan cerrada), deployeado (Fase 2), identidad visual propia "Hybrid critic notebook" (Fase 3 cerrada, ver `DESIGN.md`), 160 tests de backend. Sesión 2026-07-18: integrado "current picks" (home) y "catalog statistics" reales (footer) inspirados en el prototipo de Lovable, arreglado el mapa de afinidad (roto en prod por `datetime()` de SQLite contra Postgres) y un fix grande de performance (pool de conexiones + paralelización de llamadas a TMDb — login de ~8s a ~2.85s en producción, medido). Sesión 2026-07-20: rediseño de `/recommend` a imagen de la página real de Lovable (6 picks, grilla a 2 columnas, tilt+glare en posters, línea "Dir. X • género"), `match_score` reescrito con curva `tanh` en vez de aditivo-clampeado, fix de un bug real donde "Nuevos picks" agotaba el pool de candidatos y devolvía un error engañoso, y saqué el cartel de filas de CSV descartadas (no era un error real). **Sesión 2026-07-20 (release):** ejecutadas las Olas 1-3 del plan de implementación (warm-up del backend, rate limiting + `/admin/stats`, feedback loop real en el scoring, modo watchlist, "dónde verla", render progresivo), migrada la base de Neon de São Paulo a Oregon (login 2.85s → 0.59s medido), arreglado `/health` que devolvía 405 a monitores de uptime, y **renombrado el proyecto de PeliPick a Butaca** — 180 tests de backend. ⚠️ **Nada de esa sesión está commiteado ni deployado**, y las URLs de deploy siguen siendo `pelipick.*` a propósito (renombrar los proyectos en Vercel/Render es un paso aparte). Leer la sección `Pending` de `TASKS.md` antes de seguir. Falta: comprar dominio (`butaca.io/.co/.me/.film` libres), activar Resend, y la Ola 4 (onboarding sin Letterboxd, verificación de email + borrar cuenta, README).
